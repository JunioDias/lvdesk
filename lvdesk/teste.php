SELECT c.*, u.nome AS nome_autor FROM comunicacao_interna_movimentos AS c INNER JOIN usuarios AS u ON id_autor = u.id WHERE c.lixo = 0 AND c.id_destinatario = 4 AND c.lida = 0 GROUP BY c.id_destinatario ASC LIMIT 3

SELECT pav_inscritos.* FROM `comunicacao_interna_movimentos` INNER JOIN pav_inscritos ON pav_inscritos.protocol = comunicacao_interna_movimentos.protocol WHERE autor = '2' AND comunicacao_interna_movimentos.lixo = 0
