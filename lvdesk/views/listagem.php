  ...
  <div class="modal-content">
		  ...
		  <select class="form-control" name="id" id="select_provedor">
			...
			if(isset($resultB)){				
				while($linhas = $resultB->fetch_assoc()){
					echo "<option contrato='".$linhas['contrato_id']."' value='".$linhas['id']."' tipo='".$linhas['tipo_bd']."' >".$linhas['nome']." - ".$linhas['sistema']."</option>";
				}		
			}		  
			?>
			</select>	
	...
	<div class="modal-dialog">
	  <div class="modal-content">
		...
		<div class="modal-footer">
		  <button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Fechar</button>
		  <a id="primario" class='btn btn-success rtrn-conteudo-listagem' item='on' data-objeto='form_search_clientes' flag='<?= $connect['tipo_bd']; ?>' caminho='controllers/sys/pav.sys.php' data-dismiss="modal">Pesquisar</a>
		</div>
	  </div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
